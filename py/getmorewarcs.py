...
foundwarcs = json.loads(open("../json/foundwarcs.json").read())
...
try:
	for row in top2000csv:
		...
		if artist == "Alan Parsons Project":
			artist = "The Alan Parsons Project"

		alreadyfound = False
		for foundwarc in foundwarcs:
			if row['1999'] == foundwarc['rank']:
				alreadyfound = True
				break
		if alreadyfound:
		...
		artist = artist.lower().replace(" & ", '-').replace(". ",'-').replace(' ','-').replace(',','').replace('\'','').replace('(','').replace(')','').replace('.','-').replace('/','.').replace('!','')
		track = track.lower().replace("Part I", "Part 1").replace(" & ", '-').replace(". ",'-').replace(' ','-').replace(',','').replace('\'','').replace('(','').replace(')','').replace('.','-').replace('/','.').replace('!','')
		track += "-chords"
	...
	output = open("../json/foundmorewarcs.json", "w")
	output.write(json.dumps(result))
	print("Written to ../json/foundmorewarcs.json")
