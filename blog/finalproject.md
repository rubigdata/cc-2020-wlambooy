...
I now needed to compress the code snippets I had in Zeppelin to a single Spark Job running over all the data I had collected. Using Sparks JSON reader, I was able to create a RDD from the ```ugwarcs.json``` file I created earlier. Using the Map-Reduce design pattern, I created a mapper function ```getMostFrequentChords``` that takes a RDD row as input and outputs a list of key-value pairs. The output of running this mapper function on the input RDD was then flatMapped so a new RDD could be made. This enabled me to use the reduceByKey function, which reduces the mapped input using the different chords as keys. 
...
``` 
The final run that succeeded in giving output took a whopping 13 and a half hours, the output can be found [here]() and in the table below.
